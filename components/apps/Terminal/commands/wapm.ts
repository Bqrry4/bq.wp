...
//     const webc = (
//         (await fetchWithQuery(getPackageQuery(pkg))) as GetPackageQueryResponse
...
//     const webcBin = new Uint8Array(
//         await (await fetch(webc.webcUrl)).arrayBuffer(),
//     );

//     // Parse the wasm payload
//     const offset: WebcAtom = JSON.parse(webc.offsets).find(
//         (item: { type: string }) => item.type === "atom",
//     );
...
//     fetchWithQuery(searchQuery(pkg)).then((value: SearchQueryResponse) => {
